...
itermax = 20;                  % no of iterations
sk_docs_time = zeros(K, itermax+1);
...
for iter = 1:itermax % number of Gibbs sweeps
  ...
  sk_docs_time(:,iter+1) = sk_docs;
end

plot([0:itermax], (sk_docs_time + alpha)./(sum(sk_docs_time(:,1)+alpha)));
xlabel('Iteration'); ylabel('Mixing Proportion');
legend_str = {};
for tpk=1:K
    legend_str{end+1} = sprintf('Topic %d', tpk);
end
